from bootstrapping_olympics.unittests.manager_tests.test_basic_operations import create_tmp_dir
import os
...
def check_basic_operations(id_agent, agent, id_robot, robot):
    ...
    with create_tmp_dir() as root:
        ...
        for logs_format, interface in LogsFormat.formats.items():
            ...
            written = []
            with interface.write_stream(filename, 'example', robot.get_spec()) as writer:
            ...
            for obs_read in interface.read_from_stream(filename, 'example'):
                ...
                assert_allclose(obs_read['observations'], original['observations'])
