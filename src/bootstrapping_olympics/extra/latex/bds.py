    ...
    with latex_fragment(sys.stdout, graphics_path=graphics_path) as frag:
        ...
        with frag.figure(caption=caption, label=label, placement="p") as fig:
            ...
            with fig.subfigure(caption="$\Tcove^{sv}$",
                ...
                gid = '%s-%s-%s-P' % (id_set, robot, agent),
                save_data(report['estimator/tensors/P/value/posneg'], gid)
            ...
            for i in range(ncmd):
                ...
                with fig.subfigure(caption=subcaption, label=sublabel) as sub:
                    gid = '%s-%s-%s-T%d' % (id_set, robot, agent, i)
                    save_data(report['estimator/tensors/T/slices/%d/value/posneg' % i], gid)
                    tensor_figure(sub, gid=gid, xlabel='s', ylabel='v', width=width,
                              label='T^{%dsv}' % i)
            ...
            for i in range(ncmd):
                ...
                with fig.subfigure(caption=subcaption, label=sublabel) as sub:
                    gid = '%s-%s-%s-M%d' % (id_set, robot, agent, i)
                    save_data(report['estimator/model/M/slices/%d/value/posneg' % i], gid)
                    tensor_figure(sub, gid=gid, xlabel='s', ylabel='v', width=width,
                              label='M^{s}_{%dv}' % i)
...
def bds_learn_report_A_old(id_set, agent, robot, draft=False):
