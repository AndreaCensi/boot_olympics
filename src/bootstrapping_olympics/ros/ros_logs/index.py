...
def bag_describe_stream(bag_file, bag, topic):
    ...
    for topic, msg, t in bag.read_messages(topics=[topic]):
        ...
        if id_episode == 'id-episode-not-set':
            warnings.warn('hack: auto-assigning id_episode (bug in first '
                          'versions of BootOlympics)')
...
def bag_get_bootstrapping_stream(bag_file):
    ...
    streams = []
    bag = rosbag.Bag(bag_file) #@UndefinedVariable
    ...
    for topic in topics:
        ...
        if datatype == BootstrappingObservations_datatype:
