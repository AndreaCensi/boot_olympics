...
from blocks import SimpleBlackBox, WithQueue
from blocks.library import Route, WrapTimedNamed
from bootstrapping_olympics import (BootSpec, NuisanceNotInvertible, 
...
class PureCommandsNuisance(RepresentationNuisanceCausal):
    ...
    @contract(returns=SimpleBlackBox)
    def get_H(self):
        H0 =  PureCommandsPostFilter(self.delta)
    
        H0w = WrapTimedNamed(H0)
        H = Route([({'observations':'observations'}, 
                    H0w, {'observations':'observations'})])
        return H
...
class PureCommandsPostFilter(WithQueue):
...
class PureCommandsPreFilter(WithQueue):
